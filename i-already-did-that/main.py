...
#  such as by reaching a constant value. For example, 
# starting with n = 210022, k = 6, b = 3,
...
# since iterating on 102212 would return to 210111 when done in base 3. If the algorithm reaches a constant, such as 0, then the length is 1.

# Languages
# ==============================

# https://stackoverflow.com/a/28666223/14892663
...
def sort_(n, descending ):
    int_list = []
    for i in range(0, len(n)):
        int_list.append(int(n[i]))
    int_list.sort(reverse=descending)

    output = ""
    for i in range(0, len(int_list)):
        output += str(int_list[i])
    return output
...
def solution(n, b):
    ...
    while(ii < 100000):
        ...
        z = x_base_10 - y_base_10

        z = ''.join(map(str, numberToBase(z, b)))
...
if __name__ == "__main__":
    ...
    for case in cases:
        print("Testing", case[0], "=>", case[1], "==", case[2] , "OK" if middleware(case[0], case[1]) == case[2] else "FAIL")
